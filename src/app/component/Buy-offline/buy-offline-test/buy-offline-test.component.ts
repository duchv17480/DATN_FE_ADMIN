...
export class BuyOfflineTestComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.validFormDeliveryOrder = new FormGroup({
      'fullname': new FormControl(null, [Validators.required, Validators.minLength(6), Validators.maxLength(50)]),
  ...
  createOrder() {
    this.isLoading = true;
    this.restOrder.createAnOrderAtTheCounter().subscribe(res => {
  ...
  getAllPaymentStatus() {
    this.restOrder.getAllPaymentStatus().subscribe(res => {
      ...
      this.createDate = res.data[0].createDate;
    })
  }
